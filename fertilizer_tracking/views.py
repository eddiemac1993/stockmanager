...
def dashboard(request):
    ...
    total_today_sales = today_sales.aggregate(Sum('total_amount'))['total_amount__sum'] or 0
    total_today_commissions = today_sales.aggregate(Sum('commission_earned'))['commission_earned__sum'] or 0
    ...
    # Calculate balance
    try:
        yesterday_balance = DailyBalance.objects.filter(date=today - timedelta(days=1)).first()
        opening_balance = yesterday_balance.closing_balance if yesterday_balance else 0
    except:
        opening_balance = 0
    ...
    context = {
        ...
        'total_today_commissions': total_today_commissions,
        'stocks': stocks,
